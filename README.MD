# vps-scripts

### 演示使用github脚本搭建vps

##### 脚本使用示例：
```
//查看shell脚本内容方式：在github上找到shell脚本，点击右上角到raw，复制链接即可得到如下内容：
https://raw.githubusercontent.com/Clearlove2015/vps-scripts/main/trojan-go.sh
//上面链接可以访问的条件是该仓库必须是public
//Ubuntu执行脚本
bash <(curl -sSL "https://raw.githubusercontent.com/Clearlove2015/vps-scripts/main/trojan-go.sh")
//如果是国内的Ubuntu主机，终端需要设置代理才能执行。

xray:
https://github.com/mack-a/v2ray-agent

naiveproxy:
https://github.com/Misaka-blog/naiveproxy-script

hysteria2:
https://github.com/flame1ce/hysteria2-install

tuic(停更):
https://gitlab.com/Misaka-blog/tuic-script/

juicity(替代tuic):
https://github.com/Misaka-blog/juicity-script

mieru:
https://github.com/Misaka-blog/mieru-script

acme.sh（证书一键申请）：
https://github.com/Misaka-blog/acme-script
//前提：将Godaddy购买的域名xxx.com添加到Cloudflare
//参考：https://www.multimediaxp.com/post/154/how-to-add-a-godaddy-domain-to-cloudflare
```